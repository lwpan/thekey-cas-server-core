...
public class AbstractValueCssFilter extends AbstractStyleCssFilter {
    ...
    protected final void filterStyles(final CSSStyleDeclaration styles) {
        // for {@link CSSStyleDeclarationImpl} we can directly manipulate the Property objects to get around the
        // standard API limitations
        if (styles instanceof CSSStyleDeclarationImpl) {
            final List<Property> properties = ((CSSStyleDeclarationImpl) styles).getProperties();
            for (int j = 0; j < properties.size(); j++) {
                final Property property = properties.get(j);
                if (!this.filterValue(styles, property.getName(), property.getValue())) {
                    properties.remove(j);
                    j--;
                }
            }
            ...
            for (int j = 0; j < styles.getLength(); j++) {
                ...
                // remove any duplicate properties because the standard API doesn't provide a way to manipulate
                // properties that share the same name in a rule
                ...
                if (!this.filterValue(styles, name, styles.getPropertyCSSValue(name))) {
                    propertiesToRemove.add(name);
                }
            }
